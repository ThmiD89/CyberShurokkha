...
import os
model = joblib.load(os.path.join(os.path.dirname(__file__), 'model_uci.pkl'))

# Must match the exact column order train_model_uci.py used
FEATURE_ORDER = [
...
def predict_url_uci(url):
    print(f"Analyzing {url}... (this takes a few seconds)")
